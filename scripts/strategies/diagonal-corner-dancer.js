...
const MAX_WAIT_TRACKER_CHECKS = 10;
...
async function main() {
  ...
  console.log(
    `Diagonal corner pair: first (${diagonalCornerPair.firstCorner.x}, ${diagonalCornerPair.firstCorner.y}), second (${diagonalCornerPair.secondCorner.x}, ${diagonalCornerPair.secondCorner.y}), score: ${diagonalCornerPair.score}`,
  );
  ...
  let currentCorner = diagonalCornerPair.firstCorner;
  let nextCorner = diagonalCornerPair.secondCorner;

  while (state === "active") {
    const trackerResult = await checkCornerDirections(
      huntId,
      currentCorner.openDirections,
      state,
      logWithStep,
    );
    ...
    [currentCorner, nextCorner] = [nextCorner, currentCorner];

    const waitResult = await waitAtCorner(
      huntId,
      currentCorner.openDirections,
      state,
      logWithStep,
...
const checkCornerDirections = async (
  huntId,
  directions,
  state,
  logWithStep,
  ...
  for (const direction of directions) {
    ...
    state = trackerResult.state;
    detected = detected || trackerResult.detected;
    logWithStep(
...
const waitAtCorner = async (huntId, directions, state, logWithStep) => {
  for (
    let waitStep = 0;
    state === "active" && waitStep < MAX_WAIT_TRACKER_CHECKS;
    waitStep++
  ) {
    const direction = directions[waitStep % directions.length];
    const trackerResult = await useMotionTracker(huntId, direction);
    state = trackerResult.state;
    logWithStep(
      `used motion tracker ${direction}; detected ${trackerResult.detected}; state is ${state}`,
    ...
    if (state !== "active" || trackerResult.detected) {
      return { detected: trackerResult.detected, state };
    }
  }
...
const moveToCorner = async (
  ...
  const pathToNextCorner = await getShortestRoute(
    huntId,
    currentCorner.x,
    currentCorner.y,
    nextCorner.x,
  ...
  console.log("Path to next corner:", pathToNextCorner);

  while (state === "active" && pathToNextCorner.length > 0) {
    const direction = pathToNextCorner.shift();
    if (state !== "active") {
      break;
    }
