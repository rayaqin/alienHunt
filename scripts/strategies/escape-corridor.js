...
async function main() {
  ...
  console.log(
    `Escape corridor: ${escapeCorridor.direction}; nearby boxes: ${escapeCorridor.boxCount}`,
  );
...
const findEscapeCorridor = (grid) => {
  ...
  const canMoveStraightIntoCorridor = (direction) => {
    const { x: dx, y: dy } = DELTAS[direction];

    for (let distance = 1; distance <= ESCAPE_DISTANCE; distance++) {
      if (
        !isAvailable(
          PLAYER_START.x + dx * distance,
          PLAYER_START.y + dy * distance,
        )
      ) {
        return false;
      }
  ...
  return ESCAPE_DIRECTIONS.map((direction) => ({
    ...
    isOpen: canMoveStraightIntoCorridor(direction),
  }))
...
const moveIntoEscapeCorridor = async (
  ...
  for (
    let distance = 1;
    state === "active" && distance <= ESCAPE_DISTANCE;
    distance++
  ) {
    ...
    if (
      state !== "active" ||
      distance < TRACK_AFTER_STEPS ||
      distance >= ESCAPE_DISTANCE - 1
    ) {
      continue;
    }

    const trackerResult = await useMotionTracker(huntId, backwardsDirection);
    state = trackerResult.state;
    logWithStep(
      `used motion tracker ${backwardsDirection}; detected ${trackerResult.detected}; state is ${state}`,
    );
